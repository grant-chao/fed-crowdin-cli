...
class CrowdinV2 {
    ...
    async push() {
        ...
        const branchName = await git.getBrandName();
        const bns = branchName.split("/");
        const fileName = "keys_" + bns[bns.length - 1] + ".json";
        const sourcePath = `${config.home}/source`;
        ...
        config.paths.forEach((d) => {
            ...
            const pathFiles = utils.readDirSync(dir, config.fileTypes, config.exclude);
        ...
        const pushWord = (key) => {
            if(['(.+?)','(.*?)'].includes(key) || config.variables.includes(key) || words.includes(key)) return;
            words.push(key);
        }
        ...
        uploadStorageApi.addStorage(fileName, fs.readFileSync(oldSourceFile)).then((result) => {
            ...
            sourceFilesApi.listProjectFiles(projectId, {
                limit: 500, // 最大500，超过 500就要分页了。
            }).then((result) => {
    ...
    i18n() {
        ...
        Object.keys(languageMap).forEach((key) => {
            ...
            Object.keys(sourceObject).forEach((objectKey) => {
                ...
                const rule = /(.*?):for\((.*?)\)/g;
                if(rule.test(v)) v = v.replace(rule, (res, $1) => $1);
                v = v.replace(/\n/g,'').replace(/\\*"/g, "_u0022").replace(/\\*'/g, "_u0027");
                resultData[key] = v;
            });
            const fileData = {};
