...
class CrowdinV2 {
    ...
    async push() {
        ...
        const branchName = await gitUtils.getBrandName();
        let fileName = branchName;
        if(!['main', 'master'].includes(branchName) && !branchName.startsWith(config.branch)) {
            console.log(chalk.red(`分支 ${branchName} 不符合命名规则。`));
            console.log(chalk.green(`请输入与你当前所在分支对应的 develop 分支，如 develop/2.0.1 输入 2.0.1；develop/home 输入 home。`));
            ...
            if(name1 !== name2) {
                console.log(chalk.red(`两次输入不一致。`));
                return;
            }
            fileName = name1;
        }else{
            fileName = branchName.replace(config.branch, '');
        }
